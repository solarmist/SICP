...
;; Instead of having a single (linear) branch on the recursion tree to follow
;; both the left and right side of the multiplication tree needs to be
;; evaluated at each step, so instead of O(lg n) we have O(2^lg n) = O(n)
